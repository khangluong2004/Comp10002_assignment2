...
#define ELLIPSE_LEN 3
#define CHAR_LIMIT 37
...
void continue_after_replay(state_t **cur_state_ptr, int *cur_position_ptr, 
    int num_char_line);
...
/* PROPERTIES SECTION -------------------------------------------------------*/
/* This section is the explanation of any properties resulted from compression
that will be used in later program */

/* PROPERTIES OF THE OUTGOING ARCS FROM A NODE/ STATE */
/* By the compression algorithm, all transition, outgoing strings from 1 state: 
...
1. The string only has > 1 character after compression.
...
So, considering any compression between parent node x
...
So, after compression, the difference between the outgoing arcs
would lie only in the last characters (as they share all the rest from the 
single outgoing arc from x). This holds for all compression. Another 
consequence of this characteristic of compression is that:
...
stage_1(automaton_t *automaton_ptr){
    ...
    while (!finish_file){
        ...
        if (c == NEWLINE || c == EOF){
            ...
            /* Reduce num_char as '\n' or EOF is not included in the limit */
            num_char_line--;

            /* Replay impossible (as signaled by the finish_line flag), 
            so reset flags, cur_position, put new line and terminate for 
            this line */
            if (finish_line == TRUE){
                finish_line = FALSE;
                cur_position = 0;
...
the char and continue down the automaton).
...
generating up to the char limit of the line. */
void
...
/* Function used to print out all the remaining of the transition_str 
if the last char in prompts is in the middle of a transition string.
...
finish_transition_str(state_t **cur_state_ptr, int *cur_position, int *remain){
    ...
    the remain is reduced below 0, and will be checked to stop generation */
...
Since the compression only affect the cur_node and next_node, the path from 
the root to the node before cur_node will remain unchanged. Thus, there is 
no need to start searching from the root. 

It's more efficient to restart searching from the node before cur_node.

Except if the cur_node is the root node, in which we will recheck from 
the cur_node (which is the root). */
...
*compress_automaton(automaton_t *automaton_ptr, int steps, 
    ...
    the counter for all possible states */
    int target_states = automaton_ptr->nid - steps;
...
int
compress_states(state_t *parent, state_t *child, int *removed_frequency){
    ...
    *removed_frequency = *removed_frequency + child->freq;
    ...
    /* Free parent->outputs (freeing the nodes, and not the child state)
