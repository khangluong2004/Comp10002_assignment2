...
/* Stage 0 */
automaton_t *stage_0(unsigned int *total_char);

/* Stage 1*/
void stage_1(automaton_t *automaton_ptr);

void check_and_replay(state_t **cur_state_ptr, unsigned int *cur_position_ptr, 
    unsigned int num_char_line, char c, int *finish_line_ptr);

void continue_after_replay(state_t **cur_state_ptr, 
    unsigned int *cur_position_ptr, unsigned int num_char_line);
void finish_transition_str(state_t **cur_state_ptr, unsigned int *cur_position, 
    unsigned int *remain);
void generate_to_limit(unsigned int remain, state_t **cur_state_ptr);

/* Stage 2 */
void stage_2(automaton_t *automaton_ptr, unsigned int total_char);
automaton_t *compress_automaton(automaton_t *automaton_ptr, unsigned int steps, 
    unsigned int *frequency);
int compress_states(state_t *parent, state_t *child, unsigned int *frequency);
...
void print_ellipse(unsigned int char_limit);
void print_to_limit(char *str, unsigned int char_limit);

state_t *make_state(unsigned int id);
...
node_t *find_str(list_t *list, char search_char, unsigned int *cur_position);
...
same state: 
1. Will only differs by the last characters. 
...
automaton_t
*stage_0(unsigned int *total_char){
    ...
    unsigned int num_statements = 0; /* Statement counter */
    unsigned num_chars = 0; /* Characters counter */
    ...
    while ((c = mygetchar()) != EOF){
        ...
        If non-empty line, initialize cur_state_ptr and reset new_line flag */
        if (new_line == TRUE){
            if (c == NEWLINE){
                break;
            } else {
        ...
        /* If c is a newline, move to newline, update statements counter,
...
stage_1(automaton_t *automaton_ptr){
    ...
    unsigned int num_char_line = 0; /* Counter to check for the char limit */
    unsigned int cur_position = 0; /* Set position in the transition string */
    ...
    while (!finish_file){
        ...
        if (c == NEWLINE || c == EOF){
            ...
            /* Replay impossible or all char limit used 
...
finish_line flag. */
void
check_and_replay(state_t **cur_state_ptr, unsigned int *cur_position_ptr, 
    unsigned int num_char_line, char c, int *finish_line_ptr){
    ...
    if (found_node == NULL){
        ...
        *finish_line_ptr = TRUE;
        ...
        unsigned int remain = CHAR_LIMIT - num_char_line;
...
void
continue_after_replay(state_t **cur_state_ptr, unsigned int *cur_position_ptr, 
    unsigned int num_char_line){
...
void
finish_transition_str(state_t **cur_state_ptr, unsigned int *cur_position, 
    unsigned int *remain){
    /* Find the max node, take the string, and print up to char limit */
    ...
    unsigned int str_length = strlen(next_node->str + *cur_position);
...
void
generate_to_limit(unsigned int remain, state_t **cur_state_ptr){
    /* Keep a counter of the number of characters used */
    unsigned int used_char = 0;
...
The new frequency = Total frequency - Frequency of removed states. */

void
stage_2(automaton_t *automaton_ptr, unsigned int total_char){    
    unsigned int steps;
    ...
    unsigned int removed_frequency = 0;
    automaton_ptr = compress_automaton(automaton_ptr, steps, 
        &removed_frequency);

    unsigned int total_freq = total_char - removed_frequency;
...
1. Any new node is inserted at head, and its children will be checked 
and added if unvisited. 

2. The node is removed if all its children are visited or has no children.
The search continues until the stack is empty (no more searching/ compression)
or the number of compressions steps required is satisfied.

- For the compression stages:

Since the compression only affect the cur_node and nodes below/after it 
(in depth-first search order), the path from the root to the node before 
cur_node (if any) will remain unchanged. 
Thus, there is no need to start searching from the root. 

It's more efficient to restart searching from the node before cur_node.

Except if the cur_node is the root node (no before node), in which we will 
recheck from the cur_node (which is the root). */

automaton_t
*compress_automaton(automaton_t *automaton_ptr, unsigned int steps, 
    unsigned int *removed_frequency){
    ...
    unsigned int target_states = 0;
    if (steps < automaton_ptr->nid){
        target_states = automaton_ptr->nid - steps;
    }

    /* Set up the stack for depth-first search */
    ...
    while(!is_empty(stack) && (automaton_ptr->nid > target_states)){
        ...
        /* If nothing found, all the head's children are checked
        or it has no children, so remove the head */
        if (next_state == NULL){
...
The `parent` and `child` nodes corresponds to x and y nodes specified in 
the question.
...
compress_states(state_t *parent, state_t *child, 
    ...
    *removed_frequency = *removed_frequency + child->freq;
    ...
    the outgoing str/arc of parent to all str of child's outputs. 
...
*dynamic_strcat(char *s1, char *s2){
    ...
    unsigned int new_len = len1 + len2 + 1;
...
void
print_ellipse(unsigned int char_limit){
...
void
print_to_limit(char *str, unsigned int char_limit){
    unsigned int length_str = strlen(str);

    for (unsigned int i=0; i < char_limit && i < length_str; i++){
        putchar(str[i]);
    }
}
...
node_t
*find_str(list_t *list, char search_char, unsigned int *cur_position){
    if (list == NULL){
        return NULL;
    }
    ...
    while (cur_node != NULL){
        if (cur_node->str[*cur_position] == search_char){
            /* Increment cur_position up to 
            the length of the string (by taking mod) */
            unsigned int str_length = strlen(cur_node->str);
...
*find_max_node(list_t *list){
    ...
    while (cur_node != NULL){
        unsigned int cur_freq = cur_node->state->freq;
